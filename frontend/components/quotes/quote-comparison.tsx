...
export function QuoteComparison({ quotes, selectedQuoteId, selectingId, onSelect }: { quotes: SupplierQuote[]; selectedQuoteId: string | null; selectingId?: string; onSelect: (id: string) => void }) {
  ...
  <Button className="mt-5 w-full" variant={selectedQuoteId === quote.id ? "outline" : "default"} disabled={Boolean(selectedQuoteId) || selectingId === quote.id} onClick={() => onSelect(quote.id)}>{selectedQuoteId === quote.id ? "Selected supplier" : selectingId === quote.id ? "Selecting…" : "Select this supplier"}</Button></CardContent></Card>)}</div></section>;
}
