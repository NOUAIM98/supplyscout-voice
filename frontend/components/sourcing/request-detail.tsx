...
export function RequestDetail({ id }: { id: string }) {
  ...
  const selectionAllowed = workflow.data?.workflow_status === "awaiting_human_selection" && reservationResult === null;
  ...
  return <main className="mx-auto max-w-7xl space-y-8 px-5 py-10 lg:px-8 lg:py-12">
    ...
    {quotes.data && quotes.data.length > 0 && <div className="space-y-6">{ranking.data ? <RankingPanel ranking={ranking.data} selectedQuoteId={workflow.data?.selected_quote_id ?? null} /> : <PanelSkeleton />}<QuoteComparison quotes={quotes.data} selectedQuoteId={workflow.data?.selected_quote_id ?? null} selectionAllowed={selectionAllowed} selectingId={select.variables} onSelect={quoteId => select.mutate(quoteId)} /><ReservationStage request={request.data} quote={selectedQuote ?? null} reviewed={reservationPreview.isSuccess || status === "completed"} outcome={reservationResult?.outcome} reference={reservationResult?.supplier_reference} previewing={reservationPreview.isPending} approving={reservationApproval.isPending} onReview={() => reservationPreview.mutate()} onApprove={() => reservationApproval.mutate()} /></div>}
