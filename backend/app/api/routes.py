...
from ..schemas.quotes import RankingEntry, RankingResponse, SupplierQuoteRead
...
def approve_quote_calls(request_id: str) -> list[SupplierQuoteRead]:
    ...
    if request_id not in quotes_by_request:
        quotes_by_request[request_id] = provider.collect_quotes(
            sourcing_request, list(suppliers.values())
        )
